  ...
  const handleGoogleLoginSuccess = async (credentialResponse: any) => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/auth/google/token', {
        ...
        role: 'developer' // Hardcode the role to match the component purpose
      });
  ...
  const handleOTPVerification = async (otp: string) => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/auth/signup/verify', {
        email: tempEmail,
        otp,
        role: 'developer' // Make sure role is passed in verification
      });
  
      if (response.data.success) {
        localStorage.setItem('token', response.data.token);
        navigate('/developer/dashboard'); // Navigate to manager dashboard
        return true;
      }
    } catch (error) {
      if (error.response) {
