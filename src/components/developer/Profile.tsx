...
const Profile: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    phoneNumber: developerData?.phoneNumber || '',

  });
  const [profileImage, setProfileImage] = useState<File | null>(null);
  const [profileImageBase64, setProfileImageBase64] = useState<string | null>(null);
  const [previewImage, setPreviewImage] = useState<string | null>(developerData?.profileImage || null);

  // Update form data when developerData changes
  useEffect(() => {
    if (developerData) {
      setFormData({
        name: developerData.name || '',
      ...
      setPreviewImage(developerData.profileImage || null);
    }
  }, [developerData]);
            ...
            <Card className="border-gray-700 bg-gray-800/50 backdrop-blur-sm shadow-xl">
              <CardHeader className="border-b border-gray-700 pb-4">
                <div className="flex justify-between items-center">
                  <CardTitle className="text-2xl font-bold text-white">Developer Profile</CardTitle>
                  <Button 
                    ...
                    <Avatar className="w-32 h-32 border-4 border-gray-700 shadow-lg">
                      {previewImage ? (
                        ...
                        <AvatarFallback className="bg-gradient-to-br from-blue-600 to-purple-700 text-3xl font-bold">
                          {developerData?.name ? developerData.name.charAt(0).toUpperCase() : 'D'}
                        </AvatarFallback>
                      )}
                    </Avatar>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                      <div className="space-y-2">
                        ...
                        {isEditing ? (
                          ...
                          <div className="px-3 py-2 rounded-md bg-gray-700/50 border border-gray-600 text-white">
                            {developerData?.name || 'Not provided'}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {isEditing ? (
                          ...
                          <div className="px-3 py-2 rounded-md bg-gray-700/50 border border-gray-600 text-white">
                            {developerData?.email || 'Not provided'}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {isEditing ? (
                          ...
                          <div className="px-3 py-2 rounded-md bg-gray-700/50 border border-gray-600 text-white">
                            {developerData?.phoneNumber || 'Not provided'}
                          </div>
                        )}
                      </div>
