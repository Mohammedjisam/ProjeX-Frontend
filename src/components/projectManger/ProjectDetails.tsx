    ...
    const fetchProjectDetails = async () => {
      try {
        ...
        if (projectManagerData?._id) {
          ...
          response = await axiosInstance.get(`/project/projectmanager/${projectManagerData._id}/project/${id}`)
        ...
        if (response.data.success) {
          ...
          setError("Failed to fetch project details: " + (response.data.message || "Unknown error"))
        }
      } catch (err: any) {
        console.error("Error fetching project details:", err)
        setError(`Error fetching project details: ${err.message || "Unknown error"}`)
      } finally {
        setLoading(false)
      }
    }
              ...
              <>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                  <div>
                    ...
                    <p className="text-gray-400">Client: {project.clientName}</p>
                  </div>
                  <Badge className={`${getStatusColor(project.status)} px-3 py-1 text-xs uppercase`}>
                    {project.status}
                  </Badge>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-white">{calculateDaysLeft(project.endDate)} days</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-white">{project.projectManager?.name || "Not assigned"}</p>
                          </div>
                        </div>
                          ...
                          <div>
                            ...
                            <div className="w-full">
                              <div className="flex justify-between mb-1">
                                <span className="text-white">{project.completionPercentage || 0}%</span>
                              </div>
                              <Progress value={project.completionPercentage || 0} className="h-2 bg-gray-700" />
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="space-y-6">
                    <Card className="bg-[#252b3b] border-gray-700">
                      <CardHeader>
                        <CardTitle className="text-white text-lg">Project Manager</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-gray-400 text-sm">{project.projectManager?.email}</p>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                        ...
                        {project.comments && project.comments.length > 0 ? (
                          <div className="space-y-4">
                            {project.comments.map((comment) => (
                              <div key={comment._id} className="flex space-x-4 pb-4 border-b border-gray-700">
